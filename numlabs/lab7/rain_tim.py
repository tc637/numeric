...
def initial_conditions(u, h, ho):
    ...
    u.prev[:] = 0
    h.prev[:] = 0
    h.prev[len(h.prev) // 2] = ho # h3, if there are 4 points
...
def boundary_conditions(u_array, h_array, n_grid):
    ...
    """
    u_array[0] = 0 # u1
    u_array[n_grid-1] = 0 # un
    h_array[0] = h_array[1] # height derivative is 0 on the left
    h_array[n_grid-1] = h_array[n_grid-2] # height derivative is 0 on the right


def first_time_step(u, h, g, H, dt, dx, ho, gu, gh, n_grid):
    """Calculate the first time step values using the specified conditions
    for dt from section 4.4.
    """
    u.now[0:n_grid] = 0 # modified to include end points
    factor = gu * ho 
    midpoint = n_grid // 2
    u.now[midpoint - 1] = -factor # midpoint, so 4//2 - 1 = 1 => 2nd point
    u.now[midpoint] = factor # 4//2 = 2 => 3rd point
    
    h.now[0:n_grid] = 0 # modified to include end point
    h.now[midpoint] = h.prev[midpoint] - g * H * ho * dt ** 2 / (dx ** 2)
...
def make_graph(u, h, dt, n_time):
    ...
    ax_u.set_xlabel('u Grid Point')
    ax_h.set_xlabel('h Grid Point')
